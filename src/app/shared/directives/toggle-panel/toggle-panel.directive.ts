...
export class TogglePanelDirective implements OnInit {
  ...
  @Input('togglePanel') togglePanel!: HTMLElement;
  ...
  ngOnInit(): void {
    this.togglePanel.classList.add('bulud-toggle-panel');
  }
  ...
  toggleFormVisibility(): void {
    if (this.togglePanel) {
      ...
      togglePanels.forEach(p => {
        ...
        if (element != this.togglePanel)
          element.style.display = 'none';
      });
      const isHidden = this.togglePanel.style.display === 'none';
      this.togglePanel.style.display = isHidden ? 'block' : 'none';
