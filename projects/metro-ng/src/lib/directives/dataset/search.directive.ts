...
@Directive({
  selector: 'input[metroDatasetSearch], metro-input[metroDatasetSearch]',
  standalone: true
})
export class SearchDirective {

  @Input() metroDatasetSearch: string[] = [];
  ...
  private parseArg(q: string): string {
    let search = "";
    if (q.length > 0)
      for (let i = 0; i < this.metroDatasetSearch.length; i++) {
        if (i != 0) search += '|';
        search += `${this.metroDatasetSearch[i]}:like(${q})`;
      }
    return search;
  }
}
